...
function connection(broker: "france" | "maxime" | "sandrine", gmail = true, signature = true): CalendarConnectionStatus {
  return { broker, connected: gmail, email: `${broker}@example.com`, gmailSendEnabled: gmail, gmailSignatureEnabled: signature, birthdays: { synced: 0, pending: 0, error: 0 }, mortgageRenewals: { synced: 0, pending: 0, error: 0 }, watch: { changeVersion: 0, lastNotificationAt: null, watchActive: false, expiresAt: null } };
}
