...
const state = vi.hoisted(() => ({ contacts: [] as any[], rows: [] as any[], connections: [] as any[], calls: [] as any[], failDelete: false }));
vi.mock("../google/connection", () => ({ getGoogleConnection: vi.fn(), googleAuthenticatedRequest: async (connection: any, url: string, init: RequestInit) => {
  state.calls.push({ broker: connection.broker, url, method: init.method });
  return new Response(null, { status: init.method === "DELETE" ? state.failDelete ? 503 : 404 : 200 });
} }));
...
beforeEach(()=> {
  ...
  state.connections=["france","maxime","sandrine"].map(broker=>({broker,calendar_id:"primary"})); state.calls=[]; state.failDelete=false;
});
