export const MAX_PURCHASE_AGREEMENT_PDF_SIZE_BYTES = 20 * 1024 * 1024;

export type PurchaseAgreementUploadValidation =
  | { valid: true }
  | { valid: false; status: 400 | 413 | 415; error: string };

export function validatePurchaseAgreementPDFUpload(file: { name: string; type: string; size: number }): PurchaseAgreementUploadValidation {
  if (file.size === 0) return { valid: false, status: 400, error: "Le fichier PDF est vide." };
  if (file.size > MAX_PURCHASE_AGREEMENT_PDF_SIZE_BYTES) {
