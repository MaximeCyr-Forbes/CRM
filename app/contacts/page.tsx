...
export default function ContactsPage() {
  ...
  return (
    <main className="contacts-page contacts-premium">
      ...
      {manualStep !== "closed" && <div className="contact-modal-backdrop"><section aria-labelledby="manual-contact-title" aria-modal="true" className="contact-modal contact-modal-medium" onKeyDown={(event) => { if (event.key === "Escape" && !isSaving) { event.stopPropagation(); closeManualModal(); } }} role="dialog">
        <header className="contact-modal-header"><div><p className="section-kicker">{manualStep === "details" ? "Nouveau contact" : "Attribution obligatoire"}</p><h2 id="manual-contact-title">{manualStep === "details" ? "Ajouter un contact" : "À QUI ATTRIBUER CE CONTACT ?"}</h2></div><button autoFocus={manualStep === "details"} aria-label="Fermer" onClick={closeManualModal} type="button">×</button></header>
      ...
      {importKind && <div className="contact-modal-backdrop"><section aria-labelledby="contact-import-title" onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); setImportKind(null); } }} aria-modal="true" className="contact-modal contact-modal-medium" role="dialog">
        <header className="contact-modal-header"><div><p className="section-kicker">Analyse avant insertion</p><h2 id="contact-import-title">Importer {importKind === "csv" ? "un CSV" : "une vCard"}</h2></div><button autoFocus aria-label="Fermer" onClick={() => setImportKind(null)} type="button">×</button></header>
      ...
      {assignmentTarget && <div className="contact-modal-backdrop contact-modal-top"><section aria-labelledby="contact-assignment-title" onKeyDown={(event) => { if (event.key === "Escape" && !isSaving) { event.stopPropagation(); setAssignmentTargetId(null); } }} aria-modal="true" className="contact-modal contact-modal-medium" role="dialog"><header className="contact-modal-header"><div><p className="section-kicker">{getContactName(assignmentTarget)}</p><h2 id="contact-assignment-title">À QUI ATTRIBUER CE CONTACT ?</h2></div><button autoFocus aria-label="Fermer" onClick={() => setAssignmentTargetId(null)} type="button">×</button></header><div className="broker-choice-grid">{CONTACT_ASSIGNMENTS.map((broker) => <button className={assignmentTarget.broker === broker ? "broker-choice-current" : ""} key={broker} onClick={() => void reassignContact(assignmentTarget, broker)} type="button"><span>{BROKER_LABELS[broker]}</span><span>{assignmentTarget.broker === broker ? "✓" : "→"}</span></button>)}</div></section></div>}
      {emailContact && (
        <ContactEmailModal
