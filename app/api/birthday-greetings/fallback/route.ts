...
import { birthdayRule } from "../../../lib/birthday-greetings/service";
...
export async function PATCH(request: Request) {
  ...
  try {
    if (!body.enabled) {
      const { data, error: readError } = await getSupabaseAdmin().from("automatic_email_rules").select("id").eq("rule_type", "birthday").single();
      if (readError || !data) throw readError ?? new Error("Règle absente.");
      const { error } = await getSupabaseAdmin().rpc("update_birthday_rule", { p_id: data.id, p_values: {}, p_enabled: false });
    ...
    const invalidSyntax = /\{\{|\}\}/.test(`${rule.subjectTemplate}\n${rule.bodyTemplate}`.replace(/\{\{\s*(firstName|lastName|fullName)\s*\}\}/g, ""));
    if (body.enabled && (issues.length || invalidSyntax)) return Response.json({ error: issues[0] ?? "Variable du modèle invalide." }, { status: 400 });
    const { error } = await getSupabaseAdmin().rpc("update_birthday_rule", { p_id: rule.id, p_values: {}, p_enabled: body.enabled });
    if (error) throw error;
