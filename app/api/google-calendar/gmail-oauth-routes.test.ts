...
const state = vi.hoisted(() => ({
  connections: [] as Array<{ broker: "france" | "maxime" | "sandrine"; connected: boolean; gmailSendEnabled: boolean }>,
  exchange: vi.fn(),
  save: vi.fn(),
...
describe("routes OAuth Gmail", () => {
  beforeEach(() => {
    state.connections = [];
    state.exchange.mockReset().mockResolvedValue({ access_token: "access", expires_in: 3600, scope: "openid email https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/gmail.send" });
  ...
  it("relance OAuth si Agenda est connecté mais gmail.send absent", async () => {
    state.connections = [{ broker: "maxime", connected: true, gmailSendEnabled: false }];
    ...
    expect(location.searchParams.get("scope")?.split(" ")).toEqual(expect.arrayContaining(["openid", "email", "https://www.googleapis.com/auth/calendar.events", "https://www.googleapis.com/auth/gmail.send"]));
  });
