...
export function BirthdayFallback({ rule, connections, onChange }: { rule: AutomaticEmailRule; connections: CalendarConnectionStatus[]; onChange: (enabled: boolean) => void }) {
  ...
  const enabled = rule.triggerConfig.birthdayFallbackEnabled === true;
  async function toggle() {
    if (guard.current) return;
    guard.current = true; setBusy(true); setError(null);
    try {
      const r = await workspaceRequest("/api/birthday-greetings/fallback", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled: !enabled }) });
  ...
  return <div className="birthday-fallback"><button type="button" role="switch" aria-checked={enabled} disabled={busy} onClick={() => void toggle()}>ENVOI DE SECOURS À 17 H · {busy ? "…" : enabled ? "ON" : "OFF"}</button><small>Heure du Québec. Uniquement si personne n’a souhaité bonne fête ou cliqué « Fait ». Les actions manuelles restent disponibles.</small>{CONTACT_BROKERS.filter(b => !connections.some(c => c.broker === b && c.gmailSendEnabled && c.gmailSignatureEnabled)).map(b => <small key={b}>Gmail ou signature à connecter : {BROKER_LABELS[b]}.</small>)}{error && <p role="alert">{error}</p>}</div>;
}
