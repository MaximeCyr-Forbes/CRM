...
import CustomCampaignManualSend from "./custom-campaign-manual-send";
...
export default function CustomCampaignsSection() {
  ...
  return <section className="custom-campaigns" aria-labelledby="custom-campaigns-title">
    <div className="custom-campaigns-heading"><div className="automatic-emails-section-title"><p className="section-kicker">Personnalisés</p><h2 id="custom-campaigns-title">CAMPAGNES PERSONNALISÉES</h2><p>Créez des campagnes sur mesure et déclenchez chaque envoi manuellement. Aucun envoi automatique.</p></div><button onClick={() => void openNew()} type="button">+ NOUVELLE CAMPAGNE</button></div>
    ...
    {loading ? <p className="automatic-emails-loading">Chargement des campagnes…</p> : <div className="custom-campaign-grid">
      {campaigns.map((campaign) => <article className="custom-campaign-card" key={campaign.id}><div><span className={`automatic-email-rule-status ${campaign.status}`}>{CUSTOM_EMAIL_CAMPAIGN_STATUS_LABELS[campaign.status].toUpperCase()}</span><span aria-hidden="true">✦</span></div><h3>{campaign.name.toUpperCase()}</h3>{campaign.status === "ready" && <div className="custom-campaign-ready"><strong>PRÊTE POUR ENVOI MANUEL</strong><span>AUCUN ENVOI AUTOMATIQUE</span></div>}<dl><div><dt>Contacts</dt><dd>{campaign.contactCount}</dd></div><div><dt>Courriels</dt><dd>{campaign.stepCount}</dd></div><div><dt>Début</dt><dd>{formatDate(campaign.startDate)}</dd></div><div><dt>Durée</dt><dd>{campaign.durationDays} jour{campaign.durationDays === 1 ? "" : "s"}</dd></div><div><dt>Expéditeur</dt><dd>{CUSTOM_EMAIL_SENDER_LABELS[campaign.senderStrategy]}{campaign.senderStrategy === "fixed_broker" && campaign.fixedBroker ? ` · ${BROKER_LABELS[campaign.fixedBroker]}` : ""}</dd></div><div><dt>Mode prévu</dt><dd>{campaign.executionMode === "automatic" ? "Automatique (futur)" : "À approuver"}</dd></div></dl><div className="automatic-email-rule-actions"><button onClick={() => void openEditor(campaign.id)} type="button">CONFIGURER</button><button onClick={() => setManualCampaignId(campaign.id)} type="button">ENVOYER MAINTENANT</button><button onClick={() => void openPreview(campaign.id)} type="button">PRÉVISUALISER</button>{campaign.status !== "ready" && <button className="custom-campaign-delete" onClick={() => void deleteCampaign(campaign)} type="button">SUPPRIMER</button>}</div></article>)}
