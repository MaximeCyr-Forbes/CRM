...
export default function AutomaticEmailsPage() {
  ...
  {editing && <div className="automatic-email-modal-backdrop" role="presentation"><section onKeyDown={(event) => { if (event.key === "Escape" && !isSaving) { event.stopPropagation(); setEditing(null); } }} aria-labelledby="automatic-email-editor-title" aria-modal="true" className="automatic-email-modal automatic-email-editor" role="dialog">
    ...
    <div className="automatic-email-editor-grid">
      ...
      <label>Mode prévu<select onChange={(event) => setEditing({ ...editing, executionMode: event.target.value as AutomaticEmailRule["executionMode"] })} value={editing.executionMode}><option value="approval">À approuver</option><option value="automatic">Automatique</option></select><small>{editing.ruleType === "birthday" ? "Seul le bouton de secours à 17 h autorise l’envoi automatique Bonne fête." : "Ce choix est réservé à une activation future. Cette règle reste verrouillée."}</small></label>
