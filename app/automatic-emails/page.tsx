...
function triggerLabel(rule: AutomaticEmailRule) {
  ...
  if (rule.ruleType === "google_review") {
    const types = googleReviewTransactionTypes(rule.triggerConfig);
    const typeLabel = types.length === 2 ? "achats + ventes" : types[0] === "purchase" ? "achats seulement" : "ventes seulement";
    return `${rule.triggerConfig.delayDays ?? 3} jours après la conclusion · ${typeLabel}`;
  }
  return AUTOMATIC_EMAIL_RULE_DESCRIPTIONS[rule.ruleType];
}

function reviewTransactionSelection(rule: AutomaticEmailRule) {
  const types = googleReviewTransactionTypes(rule.triggerConfig);
  return types.length === 2 ? "purchase_sale" : types[0] ?? "";
}

function reviewTransactionTypes(value: string): AutomaticEmailTransactionType[] {
  if (value === "purchase_sale") return ["purchase", "sale"];
  return value === "purchase" || value === "sale" ? [value] : [];
}
...
function transactionTypeLabel(type: AutomaticEmailOccurrence["transactionType"]) {
  return type === "purchase" ? "Achat" : type === "sale" ? "Vente" : "Non liée";
}
...
export default function AutomaticEmailsPage() {
  ...
  {editing && <div className="automatic-email-modal-backdrop" role="presentation"><section aria-labelledby="automatic-email-editor-title" aria-modal="true" className="automatic-email-modal automatic-email-editor" role="dialog">
    <header><div><p className="section-kicker">Configuration verrouillée</p><h2 id="automatic-email-editor-title">{AUTOMATIC_EMAIL_RULE_LABELS[editing.ruleType].toUpperCase()}</h2></div><button aria-label="Fermer" onClick={() => setEditing(null)} type="button">×</button></header>
    <div className="automatic-email-editor-grid">
      <label>Nom<input onChange={(event) => setEditing({ ...editing, name: event.target.value })} value={editing.name} /></label>
      <label>Statut de configuration<select onChange={(event) => setEditing({ ...editing, status: event.target.value as AutomaticEmailRule["status"] })} value={editing.status}><option value="draft">Brouillon</option><option value="ready">Prête</option><option value="paused">En pause</option></select><small>« Prête » signifie configuration complète, jamais activée.</small></label>
      <label>Mode prévu<select onChange={(event) => setEditing({ ...editing, executionMode: event.target.value as AutomaticEmailRule["executionMode"] })} value={editing.executionMode}><option value="approval">À approuver</option><option value="automatic">Automatique</option></select><small>Ce choix sera utilisé seulement lorsque les envois automatiques seront activés dans une phase future. Le module reste incapable d’envoyer.</small></label>
      <label>Expéditeur — Contact non attribué<select onChange={(event) => setEditing({ ...editing, defaultBroker: event.target.value ? event.target.value as AutomaticEmailRule["defaultBroker"] : null })} value={editing.defaultBroker ?? ""}><option value="">Sélection obligatoire pour Prête</option>{CONTACT_BROKERS.map((broker) => <option key={broker} value={broker}>{BROKER_LABELS[broker]}</option>)}</select><small>Un Contact attribué utilise toujours le Gmail de son courtier.</small></label>
      {editing.ruleType === "google_review" && <div className="automatic-email-editor-section automatic-email-editor-wide"><h3>DÉCLENCHEMENT · TYPE DE TRANSACTION ET DÉLAI</h3><div>
        <label>Transactions admissibles<select onChange={(event) => setEditing({ ...editing, triggerConfig: { ...editing.triggerConfig, transactionTypes: reviewTransactionTypes(event.target.value) } })} value={reviewTransactionSelection(editing)}><option value="purchase_sale">Achats + ventes</option><option value="purchase">Achats seulement</option><option value="sale">Ventes seulement</option></select></label>
        <label>Envoyer après — délai<input max="365" min="0" onChange={(event) => setEditing({ ...editing, triggerConfig: { ...editing.triggerConfig, delayDays: Number(event.target.value) } })} type="number" value={editing.triggerConfig.delayDays ?? 3} /><small>Nombre de jours après la conclusion · 0 à 365.</small></label>
      </div></div>}
      ...
      <div className="automatic-email-editor-section automatic-email-editor-wide"><h3>HEURE PRÉVUE</h3><div>
        <label>Heure<input max="23" min="0" onChange={(event) => setEditing({ ...editing, sendHour: Number(event.target.value) })} type="number" value={editing.sendHour} /></label>
        <label>Minute<input max="59" min="0" onChange={(event) => setEditing({ ...editing, sendMinute: Number(event.target.value) })} type="number" value={editing.sendMinute} /></label>
      ...
      <label className="automatic-email-editor-wide">Message<textarea onChange={(event) => setEditing({ ...editing, bodyTemplate: event.target.value })} rows={8} value={editing.bodyTemplate} /></label>
    </div>
    <div className="automatic-email-variables"><strong>Variables disponibles</strong>{AUTOMATIC_EMAIL_VARIABLES[editing.ruleType].map((variable) => <code key={variable}>{`{{${variable}}}`}</code>)}</div>
    {ruleConfigurationIssues(editing).length > 0 && <div className="automatic-email-editor-warning"><strong>CONFIGURATION INCOMPLÈTE</strong>{ruleConfigurationIssues(editing).map((issue) => <span key={issue}>{issue}</span>)}</div>}
