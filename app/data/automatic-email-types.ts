...
export type AutomaticEmailTriggerConfig = {
  ...
  purchaseAnniversaryFallbackEnabled?: boolean;
  leadMonths?: number;
  delayDays?: number;
...
function triggerConfig(value: unknown, ruleType: AutomaticEmailRuleType): AutomaticEmailTriggerConfig | null {
  ...
  const allowed = ruleType === "mortgage_renewal" ? new Set(["leadMonths"])
    : ruleType === "google_review" ? new Set(["delayDays", "googleReviewUrl", "transactionTypes"])
      : ruleType === "birthday" ? new Set(["birthdayFallbackEnabled"]) : ruleType === "purchase_anniversary" ? new Set(["purchaseAnniversaryFallbackEnabled"]) : new Set<string>();
  if (Object.keys(data).some((key) => !allowed.has(key))) return null;
  if (data.purchaseAnniversaryFallbackEnabled !== undefined && typeof data.purchaseAnniversaryFallbackEnabled !== "boolean") return null;
  ...
  return {
    ...(data.purchaseAnniversaryFallbackEnabled !== undefined ? { purchaseAnniversaryFallbackEnabled: data.purchaseAnniversaryFallbackEnabled as boolean } : {}),
...
export function parseAutomaticEmailRuleDraft(value: unknown): AutomaticEmailRuleDraft | null {
  ...
  return (result.status === "ready" || result.triggerConfig.birthdayFallbackEnabled || result.triggerConfig.purchaseAnniversaryFallbackEnabled) && ruleConfigurationIssues(result).length > 0 ? null : result;
}
