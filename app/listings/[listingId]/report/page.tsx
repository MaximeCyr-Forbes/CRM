...
export default function ListingReportPage() {
  ...
  const completedTasks = tracking?.tasks.filter((task) => task.completed) ?? [];
  const remainingTasks = tracking?.tasks.filter((task) => !task.completed) ?? [];
  ...
  return <main className="listing-report-page">
    ...
    <article className="listing-report-document">
      ...
      <section className="listing-report-section" aria-labelledby="report-performance-title"><div className="listing-report-section-title"><span>02</span><h2 id="report-performance-title">ACTIVITÉ DE MISE EN MARCHÉ</h2></div>{tracking && offers ? <div className="listing-report-performance"><article><strong>{tracking.visits.length}</strong><span>Visites</span></article><article><strong>{offers.length}</strong><span>Offres</span></article><article><strong>{completedTasks.length} / {tracking.tasks.length}</strong><span>Checklist complétée</span></article><article><strong>{daysOnMarket ?? "—"}</strong><span>Jours en marché</span></article></div> : <p className="listing-report-unavailable">Certaines données de performance sont temporairement indisponibles.</p>}</section>
      ...
      <section className="listing-report-section listing-report-actions-section" aria-labelledby="report-actions-title"><div className="listing-report-section-title"><span>06</span><h2 id="report-actions-title">ACTIONS DE MISE EN MARCHÉ</h2></div>{tracking ? <div className="listing-report-actions-grid"><div><h3>ACTIONS RÉALISÉES</h3>{completedTasks.map((task) => <p key={task.id}>✓ {task.title}</p>)}{completedTasks.length === 0 && <p>Aucune action complétée pour le moment.</p>}</div><div><h3>PROCHAINES ACTIONS</h3>{remainingTasks.map((task) => <p key={task.id}>○ {task.title}</p>)}{remainingTasks.length === 0 && <p>Toutes les actions sont complétées.</p>}</div></div> : <p className="listing-report-unavailable">La checklist est temporairement indisponible.</p>}</section>
    </article>
  </main>;
}
