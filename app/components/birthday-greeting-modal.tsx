...
type Detail = { name: string; broker: CalendarBroker | null; unassigned: boolean; canSend: boolean; canDone: boolean; reason: string | null; previousError: string | null };
export function BirthdayGreetingModal({ contactId, onClose, onResolved }: { contactId: string; onClose: () => void; onResolved: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/birthday-greetings/${contactId}`, { cache: "no-store", signal: controller.signal })
  ...
  async function act(action: "done" | "send") {
    ...
    try {
      const r = await workspaceRequest(`/api/birthday-greetings/${contactId}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action }) });
  ...
  return <div className="birthday-backdrop"><div ref={dialog} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="birthday-title" className="birthday-dialog" onKeyDown={e => {
    ...
    <p className="section-kicker">Une attention personnelle</p><h2 id="birthday-title">BONNE FÊTE</h2>
    {detail ? <><h3>{detail.name}</h3><p>{detail.unassigned ? "Expéditeur par défaut" : "Courtier"} : <strong>{detail.broker ? BROKER_LABELS[detail.broker] : "À configurer"}</strong></p>{detail.reason && <p role="status">{detail.reason}</p>}{detail.previousError && detail.previousError !== detail.reason && <p>{detail.previousError}</p>}</> : !error && <p role="status">Vérification de l’anniversaire et de Gmail…</p>}
