...
export function ListingOffers({ listing, ownerNames, onChanged }: {
  ...
  return <section className="listing-tracking-panel listing-offers" aria-labelledby="listing-offers-title">
    ...
    <div className="listing-offer-list">{offers.offers.map((offer) => {
      ...
      return <article key={offer.id}><header><div><strong>{formatListingAmount(offer.amount, offer.purpose)}</strong><span className={`listing-offer-status listing-offer-status-${offer.status}`}>{LISTING_OFFER_STATUS_LABELS[offer.status]}</span></div><div><button aria-label="Modifier l’offre" onClick={() => setEditing(offer)} type="button">✎</button><button aria-label="Supprimer l’offre" className="listing-task-delete" disabled={linked} onClick={() => setDeleting(offer)} type="button">⌫</button></div></header><p><span>Date</span>{displayDate(offer.offerDate)}</p>{offer.buyerNames && <p><span>{offer.purpose === "sale" ? "Acheteurs" : "Locataires"}</span>{offer.buyerNames}</p>}{(offer.collaboratingBrokerName || offer.collaboratingBrokerAgency) && <p><span>Courtier collaborateur</span>{[offer.collaboratingBrokerName, offer.collaboratingBrokerAgency].filter(Boolean).join(" — ")}</p>}{offer.notes && <blockquote>{offer.notes}</blockquote>}
        {offer.status === "accepted" && offer.purpose === "rental" && <strong className="listing-rental-accepted">OFFRE DE LOCATION ACCEPTÉE</strong>}
        {offer.status === "accepted" && offer.purpose === "sale" && !linked && !offers.transactionLink && <button className="listing-create-transaction" disabled={offers.isSaving} onClick={() => setConverting(offer)} type="button">CRÉER LA TRANSACTION</button>}
        {linked && <span className="listing-offer-linked">Liée à la transaction</span>}
      </article>;
