...
create table if not exists public.transaction_deadlines (
  ...
  constraint transaction_deadlines_source_type_check check (
    source_type in ('manual', 'oaciq')
  ),
  constraint transaction_deadlines_source_confidence_check check (
    source_confidence is null or source_confidence in ('high', 'medium', 'low')
  )
);
...
begin
  ...
  insert into public.listings (
    ...
    nullif(p_values->>'contractSignedDate', '')::date,
...
-- Écritures atomiques Contact, adresses et notes.
...
-- Broker-aware birthday workflow
begin;
...
-- Birthday scheduler (activated operationally after deployment)
begin;
